<x-layouts.guru-admin title="Dashboard Admin">
    <div class="space-y-7">
        <div><p class="text-sm font-semibold text-tosca-ink">Ringkasan sekolah</p><h1 class="mt-1 text-3xl text-navy">Selamat datang, {{ auth()->user()->name }}</h1></div>
        @if ($pendingGuardianRequests > 0)
            <a href="{{ route('admin.guardians.index') }}" class="flex items-center gap-4 rounded-card border border-orange/40 bg-orange/10 p-5 transition hover:border-orange" wire:navigate>
                <span class="flex size-11 shrink-0 items-center justify-center rounded-xl bg-orange text-navy"><flux:icon.home-modern class="size-5" /></span>
                <span class="min-w-0 flex-1">
                    <strong class="block text-navy">{{ $pendingGuardianRequests }} permintaan orang tua menunggu persetujuan</strong>
                    <span class="mt-1 block text-sm text-ink-soft">Pastikan pendaftar benar wali siswa sebelum menyetujui.</span>
                </span>
