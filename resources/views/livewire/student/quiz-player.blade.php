<div class="mx-auto max-w-3xl space-y-6">
    @if ($attempt->submitted_at)
        <x-theme.card class="py-12 text-center"><span class="mx-auto flex size-14 items-center justify-center rounded-full bg-tosca text-navy"><flux:icon.check class="size-7" /></span><h1 class="mt-5 text-2xl">Kuis sudah dikumpulkan</h1><p class="mt-2 text-sm text-ink-soft">{{ $attempt->score !== null ? 'Skor kamu: '.$attempt->score : 'Jawaban esai sedang menunggu penilaian guru.' }}</p><x-theme.button href="{{ route('student.learning.index', ['classSubjectId' => $quiz->class_subject_id, 'tab' => 'quizzes']) }}" class="mt-6">Kembali ke ruang belajar</x-theme.button></x-theme.card>
    @else
