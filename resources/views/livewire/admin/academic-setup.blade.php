<div>
    ...
    @elseif ($tab === 'subjects')
        <div class="mt-6 grid gap-6 xl:grid-cols-[0.7fr_1.3fr]">
            <x-theme.card class="self-start"><h2 class="text-lg">{{ $subjectId ? 'Ubah mata pelajaran' : 'Tambah mata pelajaran' }}</h2><form wire:submit="saveSubject" class="mt-5 space-y-4"><label class="block text-sm font-semibold text-navy">Nama<input wire:model="subjectName" placeholder="Matematika" class="mt-2 w-full rounded-xl border border-line px-3.5 py-2.5 text-sm"></label><div class="grid grid-cols-2 gap-3"><label class="block text-sm font-semibold text-navy">Kode<input wire:model="subjectCode" placeholder="MAT" class="mt-2 w-full rounded-xl border border-line px-3.5 py-2.5 text-sm uppercase"></label><label class="block text-sm font-semibold text-navy">KKM<input wire:model="subjectKkm" type="number" min="0" max="100" class="mt-2 w-full rounded-xl border border-line px-3.5 py-2.5 text-sm"></label></div>@error('subjectKkm')<p class="text-sm text-red-600">{{ $message }}</p>@enderror<x-theme.button type="submit" class="w-full">Simpan mata pelajaran</x-theme.button></form></x-theme.card>
            <div class="grid content-start gap-4 sm:grid-cols-2">@forelse($subjects as $subject)<x-theme.card><div class="flex items-start justify-between gap-3"><div><x-theme.badge tone="tosca-soft">{{ $subject->code }}</x-theme.badge><h3 class="mt-3 text-lg">{{ $subject->name }}</h3><p class="mt-1 text-xs text-ink-soft">KKM {{ $subject->kkm }}</p></div><div class="flex"><button wire:click="editSubject({{ $subject->id }})" class="p-2 text-ink-soft"><flux:icon.pencil-square class="size-4" /></button><button wire:click="deleteSubject({{ $subject->id }})" wire:confirm="Hapus mata pelajaran ini?" class="p-2 text-red-500"><flux:icon.trash class="size-4" /></button></div></div></x-theme.card>@empty<x-theme.card class="text-sm text-ink-soft sm:col-span-2">Belum ada mata pelajaran.</x-theme.card>@endforelse</div>
        </div>
