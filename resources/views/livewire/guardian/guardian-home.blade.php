<div class="space-y-7">
    <section class="relative overflow-hidden rounded-card bg-gradient-to-br from-navy to-navy-mid px-5 py-7 text-white sm:px-8 sm:py-9">
        ...
        <div class="relative">
            ...
            <p class="mt-2 text-sm text-white/70">Pantau jadwal, kehadiran, dan tugas anak di sekolah.</p>
        </div>
    </section>
    ...
    @if ($overview)
            ...
            <x-theme.card>
                ...
                <ul class="mt-5 divide-y divide-line text-sm">
                    @forelse ($overview['attendanceRecords'] as $record)
                        {{-- Selalu pakai @php versi blok di file ini: Blade memasangkan @php
                             pertama dengan @endphp pertama, jadi mencampur versi satu baris
                             dan versi blok akan menelan markup di antaranya. --}}
                        @php
        ...
        <div class="grid gap-6 lg:grid-cols-2">
                    ...
                    @forelse ($overview['assignments'] as $assignment)
                        @php
                            [$assignmentTone, $assignmentLabel] = match (true) {
                                (bool) $assignment->submitted => ['tosca-soft', 'Sudah dikumpulkan'],
            ...
            <x-theme.card :padding="false" class="overflow-hidden">
                <div class="border-b border-line px-5 py-5 sm:px-6">
                    ...
                    <p class="mt-1 text-xs text-ink-soft">Status pengerjaan, tanpa skor.</p>
                </div>
                <ul class="divide-y divide-line">
                    @forelse ($overview['quizzes'] as $quiz)
                        @php
                            [$quizTone, $quizLabel] = match (true) {
                                (bool) $quiz->completed => ['tosca-soft', 'Sudah dikerjakan'],
        ...
        <p class="text-xs text-ink-soft">Nilai dan catatan guru belum ditampilkan untuk akun orang tua.</p>
    @endif
    ...
    <div class="grid gap-6 lg:grid-cols-[1.1fr_.9fr]">
        <x-theme.card>
            ...
            <p class="mt-1 text-sm leading-6 text-ink-soft">Isi NISN atau NIS serta nama lengkap anak sesuai rapor. Sekolah akan memastikan Anda wali siswa tersebut sebelum jadwal dan kehadirannya bisa dilihat.</p>
